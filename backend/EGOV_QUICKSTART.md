...
### 0️⃣ Demo mode (без EGOV API KEY)

Если ключа нет, можно сразу запустить demo режим:

```env
EGOV_MOCK=true
DEMO_MODE=true
# EGOV_API_KEY можно оставить пустым
```

В этом режиме все `/api/egov/*` функции возвращают предсказуемые mock-ответы,
поэтому фронтенд и сценарии подачи/проверки работают для демо и тестов.
