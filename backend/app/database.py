...
if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
    print("❌ Критическая ошибка: Supabase credentials не найдены в .env!")
    print("   • SUPABASE_URL:", "✓" if SUPABASE_URL else "✗ отсутствует")
    print("   • SUPABASE_SERVICE_KEY:", "✓" if SUPABASE_SERVICE_KEY else "✗ отсутствует")
else:
    print("✓ Supabase credentials найдены")
...
if SUPABASE_SERVICE_KEY and SUPABASE_SERVICE_KEY.count(".") != 2:
    print("⚠ ВНИМАНИЕ: SUPABASE_SERVICE_KEY имеет неправильный формат!")
...
def get_db():
    ...
    try:
        ...
        if "Could not find the table" in str(e):
            print("⚠ Таблицы Supabase не найдены. Используется демо-режим.")
            print("  ℹ Запустите schema.sql в Supabase SQL Editor для создания таблиц")
            return MockDB()
        raise
